...
class GetDeepChemCocrystalDataset(object):
    def __init__(self, mol_blocks='Mol_Blocks.dir', cc_table='CC_Table/CC_Table.tab', removeHs=False):
        ...
        exception = {'BOVQUY', 'CEJPAK', 'GAWTON', 'GIPTAA', 'IDIGUY', 'LADBIB01', 'PIGXUY', 'SIFBIT', 'SOJZEW',
                     'TOFPOW',
        ...
        for line in table:
            ...
            if c1 == None:
                try:
                    ob_mol = pybel.readstring('sdf', mol_block[line[0]])
                    smi = ob_mol.write('smi').split()[0]  # 获取SMILES
                    c1 = Chem.MolFromSmiles(smi)
                    if c1:
                        c1 = Chem.AddHs(c1)
                        AllChem.EmbedMolecule(c1)
                except:
                    print(f"Warning: Could not process molecule {line[0]} for tag {tag}")
                    continue

            if c2 == None:
                try:
                    ob_mol = pybel.readstring('sdf', mol_block[line[1]])
                    smi = ob_mol.write('smi').split()[0]
                    c2 = Chem.MolFromSmiles(smi)
                    if c2:
                        c2 = Chem.AddHs(c2)
                        AllChem.EmbedMolecule(c2)
                except:
                    print(f"Warning: Could not process molecule {line[1]} for tag {tag}")
                    continue

            if c1 is None or c2 is None:
                print(f"Skipping {tag}: failed to create molecules")
                continue
...
CCDataset = GetDeepChemCocrystalDataset(mol_blocks=abs_path+'/data/Mol_Blocks.dir', cc_table=abs_path+'/data/CC_Table/CC_Table.tab')
