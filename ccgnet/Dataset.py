        ...
        for name in dataset.__dict__:
            if name in input_attr_names:
                if len(dataset.__dict__[name].shape) == 1:
                    shape = [None]
                    self.__dict__[name] = tf.placeholder(
                        tf.as_dtype(dataset.__dict__[name].dtype),
                        shape=shape,
                        name=name + '_input'
                    )
                else:
...
class Dataset(object):
    ...
    def make_graph_dataset(self, Desc=0, A_type='OnlyCovalentBond', hbond=0, pipi_stack=0, contact=0,
        ...
        if save_name:
            if 'global_state' in self.__dict__:
                np.savez(save_name, V=self.V, A=self.A, labels=self.labels, masks=self.masks,
                         graph_size=self.graph_size, tags=self.tags, global_state=self.global_state,
                         subgraph_size=self.subgraph_size)
            else:
                np.savez(save_name, V=self.V, A=self.A, labels=self.labels, masks=self.masks,
                         graph_size=self.graph_size, tags=self.tags, subgraph_size=self.subgraph_size)
        if make_dataframe:
            self.dataframe = {}
    ...
    def make_embedding_dataset(self, fp_type='ecfp', nBits=2048, processes=15, make_dataframe=True, radii=2,
                               save_name=None):
        exception = {'BOVQUY', 'CEJPAK', 'GAWTON', 'GIPTAA', 'IDIGUY', 'LADBIB01', 'PIGXUY', 'SIFBIT', 'SOJZEW',
                     'TOFPOW',
                     'QOVZIK', 'RIJNEF', 'SIBFAK', 'SIBFEO', 'TOKGIJ', 'TOKGOP', 'TUQTEE', 'BEDZUF'}
        ...
        for i in D:
            for j in i:
                #attrs[j].append(i[j])
                if isinstance(attrs[j], list):
                    attrs[j].append(i[j])
                else:
                    attrs[j] = np.append(attrs[j], i[j])
        for l in attrs:
            if l in self.data_attr_names:
                attrs[l] = np.array(attrs[l])
        del D
